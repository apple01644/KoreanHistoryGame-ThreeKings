...
void draw_find(int x, int y)
{
	...
	for (int i = 0; i + t < 320 && k != man.end(); i += 40)
	{
		...
		if (k._Ptr->_Myval.live)
		{
			...
			draw_string(0, k._Ptr->_Myval.name,Color["c_white"],&p,30, 0.77 ,left_align + middle_align);

			p.x += 100;
			
			draw_string(0, std::to_string(year - k._Ptr->_Myval.born_year + 1), Color["c_white"], &p, 30, 0.5, center_align + middle_align);

			p.x += 30;

			draw_string(0, get_ideology(k._Ptr->_Myval.fascist, k._Ptr->_Myval.liberty), Color["c_white"], &p, 30, 0.77, left_align + middle_align);
		}
		k++;
	}
	...
	p.x = r.x + r.w / 2;
	p.y = r.y + r.h / 2;
	draw_string(2, "검색", Color["c_white"], &p, 18, 0.7,center_align + middle_align);
	...
	p.x = r.x + r.w / 2;
	p.y = r.y + r.h / 2;
	draw_string(2, "취소", Color["c_white"], &p, 18, 0.7, center_align + middle_align);
...
void draw_item(std::reverse_iterator<std::vector<Widget>::iterator> I, const SDL_Rect r, const unsigned char type)
{
	if (type == wd_none)
	{
		if (I->avail_ev)
